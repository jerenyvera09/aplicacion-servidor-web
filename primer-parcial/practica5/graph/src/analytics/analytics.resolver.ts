...
import {
  AreaRankingType,
  EtiquetaRankingType,
  UsuarioDashboardType,
  UsuarioAportesRankingType,
  ReportePromedioType,
  EtiquetaCoocurrenciaType,
  TendenciaTemporalType,
  AreaCategoriaConteoType,
...
export class AnalyticsResolver {
  ...
  async topAreasConMasReportes(
    ...
    for (const { area, reportesIds } of map.values()) {
      const puntuacionesArea = puntuaciones.filter((p) =>
        reportesIds.includes(p.reporte?.id || p.reporteId),
      );
      const total = reportesIds.length;
      const promedio = puntuacionesArea.length
        ? puntuacionesArea.reduce(
            (s, p: any) => s + (Number(p.valor) || 0),
            0,
          ) / puntuacionesArea.length
        : 0;
    ...
    list.sort(
      (a, b) =>
        ...
        (b.promedioPuntuacion || 0) - (a.promedioPuntuacion || 0),
    );
    return typeof limite === 'number'
      ? list.slice(0, Math.max(0, limite))
      : list;
  }
  ...
  async tableroEtiquetas(
    ...
    for (const e of etiquetas) byId.set(e.id, e);

    const counts = new Map<
      number,
      { reportesUsos: number; comentariosUsos: number }
    >();
    ...
    for (const r of reportes) {
      ...
      for (const e of etis) {
        if (!counts.has(e.id))
          counts.set(e.id, { reportesUsos: 0, comentariosUsos: 0 });
    ...
    list.sort(
      (a, b) => b.totalUsos - a.totalUsos || b.reportesUsos - a.reportesUsos,
    );
    return typeof limite === 'number'
      ? list.slice(0, Math.max(0, limite))
      : list;
  ...
  async resumenUsuario(
    ...
    const reportesUsuario = reportes.filter((r) => r.usuario?.id === usuarioId);
    const comentariosUsuario = comentarios.filter(
      (c) => c.usuario?.id === usuarioId,
    );
    const reportesIds = new Set(reportesUsuario.map((r) => r.id));
    const puntuacionesDeSusReportes = puntuaciones.filter((p) =>
      reportesIds.has(p.reporte?.id || p.reporteId),
    );
    const promedio = puntuacionesDeSusReportes.length
      ? puntuacionesDeSusReportes.reduce(
          (s, p: any) => s + (Number(p.valor) || 0),
          0,
        ) / puntuacionesDeSusReportes.length
      : 0;
  ...
  async rankingUsuariosPorAportes(
    ...
    @Args('desde', {
      type: () => String,
      nullable: true,
      description: 'Fecha ISO inclusiva para comentarios',
    })
    desde?: string,
    @Args('hasta', {
      type: () => String,
    ...
    for (const p of puntuaciones) {
      ...
      if (!uid) continue;
      puntuacionesPorUsuario.set(
        uid,
        (puntuacionesPorUsuario.get(uid) ?? 0) + 1,
      );
    }
    ...
    for (const id of ids) {
      const usuario =
        byId.get(id) ??
        ({ id, nombre: `Usuario ${id}`, email: '', estado: '' } as any);
    ...
    return typeof limite === 'number'
      ? ranking.slice(0, Math.max(0, limite))
      : ranking;
  }
  ...
  async reportesPorEtiquetaConPromedio(
    ...
    const reportesEtiqueta = (reportes as ReporteType[]).filter((r) => {
      const etis = (r as any).etiquetas as Array<{ id: number }> | undefined;
      return Array.isArray(etis) && etis.some((e) => e.id === etiquetaId);
    });

    const list: ReportePromedioType[] = [];
    for (const r of reportesEtiqueta) {
      const ps = puntuaciones.filter(
        (p) => (p.reporte?.id || p.reporteId) === r.id,
      );
      const promedio = ps.length
        ? ps.reduce((s, p: any) => s + (Number(p.valor) || 0), 0) / ps.length
        : 0;
  ...
  async etiquetasCoocurrentes(
    ...
    const counts = new Map<
      number,
      { enReportes: number; enComentarios: number }
    >();
    ...
    for (const r of reportes) {
      ...
      for (const e of etis) {
        ...
        if (!counts.has(e.id))
          counts.set(e.id, { enReportes: 0, enComentarios: 0 });
    ...
    for (const [id, v] of counts.entries()) {
      ...
      list.push({
        etiqueta,
        enReportes: v.enReportes,
        enComentarios: v.enComentarios,
        total: v.enReportes + v.enComentarios,
      });
    }
  ...
  async reportesPorAreaYCategoria(
    ...
    const map = new Map<
      string,
      { area: any; categoria: any; cantidad: number }
    >();
    for (const r of reportes) {
      const area = r.area;
  ...
  async buscadorReportesAvanzado(
    @Args('filtro', { type: () => BusquedaAvanzadaInput, nullable: true })
    filtro?: BusquedaAvanzadaInput,
    ...
    if (filtro?.usuarioId)
      items = items.filter((r) => r.usuario?.id === filtro.usuarioId);
    if (filtro?.etiquetaId)
      items = items.filter(
        (r) =>
          Array.isArray(r.etiquetas) &&
          r.etiquetas.some((e: any) => e.id === filtro.etiquetaId),
      );
