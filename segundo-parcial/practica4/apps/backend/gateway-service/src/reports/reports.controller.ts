...
export class ReportsController {
  ...
  async create(@Body() dto: CreateReportDto): Promise<ServiceResponse<Report>> {
    ...
    try {
      ...
      if (response.data) {
        const eventPayload = { report: response.data, request: dto };
        this.webhookEmitter.emit('reporte.creado', eventPayload).catch(() => undefined);
        const priority = String(response.data.priority ?? '').toUpperCase();
        if (['HIGH', 'CRITICAL', 'URGENT'].includes(priority)) {
          this.webhookEmitter.emit('reporte.critico', eventPayload).catch(() => undefined);
        }
      }
